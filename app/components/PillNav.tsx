...
const PillNav: React.FC<PillNavProps> = ({
  ...
  useEffect(() => {
    ...
    const layout = () => {
      circleRefs.current.forEach((circle) => {
        ...
        const D = Math.ceil(2 * R) + 20;
        const delta =
          Math.ceil(R - Math.sqrt(Math.max(0, R * R - (w * w) / 4))) + 10;
        ...
        tl.to(
          circle,
          { scale: 2, xPercent: -50, duration: 2, ease, overwrite: "auto" },
          0
        );
        if (label) {
          tl.to(
  ...
  }, [items, ease, initialLoadAnimation, isMounted, showLogo]);
  ...
  return (
    <div className={`pill-nav-container ${!showLogo ? "centered" : ""}`}>
      <nav
